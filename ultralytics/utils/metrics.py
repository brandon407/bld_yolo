...
def focal_inner_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0,
              gamma=0.5, ratio=1.0):
    ...
    with torch.no_grad():
        alpha = v / (v - iou + (1 + eps))
    iou = torch.pow(iou, gamma) * (iou - ((rho2 / c2) + (alpha * v)))
    ...
    if CIoU or DIoU or GIoU or EIoU:
        ...
        if CIoU or DIoU or EIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return inner_iou - (rho2 / c2 + v * alpha)  # CIoU
            elif EIoU:
        ...
        return inner_iou - (c_area - union) / c_area  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    elif SIoU:
        ...
        return inner_iou - d1 / mpdiou_hw_pow - d2 / mpdiou_hw_pow  # MPDIoU
    elif PowerfulIoU:
        ...
        dh2 = torch.abs(b1_y2.maximum(b1_y1) - b2_y2.maximum(b2_y1))
        P = ((dw1 + dw2) / torch.abs(w2) + (dh1 + dh2) / torch.abs(h2)) / 4
        piou = 1 - inner_iou - torch.exp(-P ** 2) + 1
        q = torch.exp(-P)
        x = q * 1.2
        return 1 - 3 * x * torch.exp(-x ** 2) * piou

    return inner_iou  # IoU


def focal_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0,
              gamma=0.5):
...
def focaler_inner_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0,
                d=0.0, u=0.95, ratio=1.0,):
    ...
    # IoU
    iou = ((inter / union - d) / (u - d)).clamp(0, 1)
...
def focaler_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0,
                d=0.0, u=0.95):
...
def inner_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0,
              ratio=1.0, ):
...
def bbox_iou(box1, box2, xywh=True, EIoU=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, shapeIoU=False,
             MDPIoU = False, PowerfulIoU=False, PowerfulIoUv2=False, feat_h=640, feat_w=640, eps=1e-7, scale=0):
