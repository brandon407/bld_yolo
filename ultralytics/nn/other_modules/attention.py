...
__all__ = ['CBAM', 'BAM', 'ECA', 'SimAM', 'A2', 'CoordAtt', 'GAM_Attention', 'SKAttention', 'SE', 'LSKblock', 'Mlp',
           'ACmix', 'iRMB', 'EfficientAttention', 'EMA', 'Attention_xy', 'OmniAttention', 'PolaLinearAttention',
           'DiSpAM']

# DarkIR: Robust Low-Light Image Restoration
# https://blog.csdn.net/StopAndGoyyy?spm=1011.2124.3001.5343
class LayerNormFunction_DiSpAM(torch.autograd.Function):
    @staticmethod
...
class LayerNorm2d_DiSpAM(nn.Module):
    def __init__(self, channels, eps=1e-6):
        super(LayerNorm2d_DiSpAM, self).__init__()
        self.register_parameter('weight', nn.Parameter(torch.ones(channels)))
        self.register_parameter('bias', nn.Parameter(torch.zeros(channels)))
        self.eps = eps

    def forward(self, x):
        return LayerNormFunction_DiSpAM.apply(x, self.weight, self.bias, self.eps)
...
class DiSpAM(nn.Module):
    ...
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, dilations=[1], extra_depth_wise=False):
        ...
        self.norm1 = LayerNorm2d_DiSpAM(c)
...
class PolaLinearAttention(nn.Module):
    def __init__(self, dim, num_patches=2, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 sr_ratio=1,
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(size=(1, 1, dim)))
